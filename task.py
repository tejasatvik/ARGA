...
class Task:
    ...
    def initialize_frontier(self):
        ...
        # optionally restrict to a provided list of abstractions
        # If `self.allowed_abstractions` exists (set externally) use that,
        # otherwise fall back to all_possible_abstractions (original behavior).
        if hasattr(self, "allowed_abstractions") and self.allowed_abstractions:
            # validate provided names against the known abstractions
            invalid = [a for a in self.allowed_abstractions if a not in self.all_possible_abstractions]
            if invalid:
                print("ERROR: The following requested abstractions are invalid: {}".format(invalid))
                print("Valid abstraction names are: {}".format(self.all_possible_abstractions))
                raise ValueError("Invalid abstraction name(s) requested.")
            abstractions_to_iterate = self.allowed_abstractions
            print("Restricting search to allowed_abstractions: {}".format(abstractions_to_iterate))
        else:
            abstractions_to_iterate = self.all_possible_abstractions

        for abstraction in abstractions_to_iterate:
